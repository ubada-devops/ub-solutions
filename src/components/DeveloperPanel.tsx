...
export const DeveloperPanel: React.FC<DeveloperPanelProps> = ({
  ...
  const devAlias = currentUser?.full_name || 'UB_DEV_14';

  // Filter tasks assigned to devAlias
  const myTasks = tasks.filter(t => t.assignedDev === devAlias || t.assignedDev === 'UB_DEV_14');
  ...
  // Submit code for CTO validation
  const handleSubmitReview = (e: React.FormEvent) => {
    ...
    setTasks(prev => prev.map(t => {
      if (t.id === selectedTaskToSubmit) {
        addToast(`Codebase submitted. Notifying CTO for PR review validation.`, 'success');
  ...
  return (
    <div className="space-y-8 max-w-4xl mx-auto py-4">
      ...
      {taskCapacity >= 4 && (
        <div className="bg-yellow-950/60 border-2 border-yellow-500 text-yellow-200 p-6 rounded flex flex-col md:flex-row items-center justify-between gap-4 shadow-lg">
      ...
      <div className="flex bg-zinc-900 border border-zinc-800 p-1 rounded">
        <button
          onClick={() => setActiveTab('workbench')}
          className={`flex-1 py-3 text-sm font-bold uppercase flex items-center justify-center gap-2 transition-all rounded ${activeTab === 'workbench' ? 'bg-emerald-500 text-black shadow' : 'text-zinc-400 hover:text-zinc-200'}`}
        >
          <LayoutGrid size={16} />
          Active Workbench & Sprints logs
        </button>
        <button
          onClick={() => setActiveTab('deliveries')}
          className={`flex-1 py-3 text-sm font-bold uppercase flex items-center justify-center gap-2 transition-all rounded ${activeTab === 'deliveries' ? 'bg-emerald-500 text-black shadow' : 'text-zinc-400 hover:text-zinc-200'}`}
        >
          <Eye size={16} />
      ...
      {activeTab === 'workbench' ? (
          ...
          <div className="p-6 bg-zinc-950/40 border border-zinc-800 rounded space-y-4">
            ...
            {myTasks.length === 0 ? (
              ...
              <div className="space-y-3">
                {myTasks.map((t) => (
                  <div key={t.id} className="p-4 bg-zinc-900 border border-zinc-850 flex justify-between items-center rounded text-xs">
                    <div>
                      <div className="flex items-center gap-2">
                        <span className="text-[10px] bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 px-1.5 py-0.5 font-bold font-mono uppercase rounded-sm">{t.id}</span>
                        <span className="text-[10px] text-zinc-400 font-bold font-mono">Stage: {t.stage}</span>
                      </div>
                    ...
                    <div className="flex items-center gap-3">
                      <span className={`text-[9px] px-2 py-0.5 border font-bold uppercase rounded ${t.health === 'Stable' ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' : 'bg-red-500/10 text-red-400 border-red-500/20'}`}>
                        {t.health}
                      </span>
                    </div>
                  </div>
        ...
        <div className="bg-zinc-900/60 border border-zinc-800/80 rounded p-8 space-y-8 shadow-2xl backdrop-blur-md">
          ...
          <div className="border-b border-zinc-800 pb-6">
            ...
            <p className="text-sm text-zinc-400 mt-1">Submit completed codebases to CTO for review, and claim new projects from the open leads queue.</p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <form onSubmit={handleSubmitReview} className="p-6 bg-zinc-950/40 border border-zinc-800 rounded space-y-4">
              ...
              <p className="text-xs text-zinc-500">Pushes active commits to staging pipelines and triggers pull requests automatically for evaluation.</p>
              ...
              <div className="space-y-3">
                <div className="space-y-1">
                  ...
                  <select
                    ...
                    className="w-full bg-zinc-900 border border-zinc-800 text-xs px-3 py-2.5 outline-none focus:border-emerald-500 text-zinc-300 font-mono rounded"
                  >
                  ...
                  leads.map((lead) => (
                    <div key={lead.id} className="p-3 bg-zinc-900 border border-zinc-850 flex justify-between items-center rounded text-xs font-mono">
                      ...
                      <button
                        ...
                        className="px-3 py-1 bg-zinc-950 hover:bg-emerald-500 border border-zinc-800 hover:border-emerald-500 disabled:opacity-50 text-emerald-400 hover:text-black font-bold uppercase tracking-wider text-[10px] transition-colors cursor-pointer rounded"
                      >
                        Claim
                      </button>
                    </div>
            ...
            <div className="p-6 bg-zinc-950/40 border border-zinc-800 rounded space-y-4">
              ...
              <div className="space-y-3">
                <select
                  ...
                  className="w-full bg-zinc-900 border border-zinc-800 text-xs px-3 py-2 outline-none focus:border-emerald-500 text-zinc-300 font-mono rounded"
                >
